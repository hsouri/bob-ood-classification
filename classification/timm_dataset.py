...
def create_other_dataset(
    ...
    if name in transfer_datasets:
        ds = get_transfer_datasets(name, root, 'train' if split=='train' else 'val', json_path)
        ...
        import tllib.vision.datasets as tldatasets
        ds_class = tldatasets.__dict__['VisDA2017']
        ds_class.download_list = []
        ds = ds_class(root, 'Synthetic' if split=='train' else 'Real', download=False)
    else:
